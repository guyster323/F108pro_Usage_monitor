...
- **설정 앱**은 계정 체크·별명·시간·한/영·미리보기·수동 업로드를 담당합니다. 지속적인 자동 조회·업로드는 `quotadeck run`을 사용하세요.
...
트레이는 앱 열기와 수동 업로드를 제공합니다. `Windows 시작 시 실행`은 설정 앱만 자동 실행하며,
현재 장기 자동 조회·업로드는 CLI의 `quotadeck run`에서 동작합니다.
### CLI
...
| 60분 | 약 16회 | 약 17년 |
| 1분 | 약 960회 | 약 3.4개월 |
100회 counter는 하나의 장기 실행 `quotadeck run` 프로세스 안에서만 동작하며 디스크에
영속화되지 않습니다. 프로세스를 다시 시작하거나 설정 앱에서 **지금 키보드에 올리기**를
반복하는 경우의 안전장치가 아니므로, 수명 계산은 위의 간격 기준을 따르고 수동 업로드를
연속 실행하지 마세요.
