...
- **설정 앱**은 계정 체크·별명·미리보기·지금 올리기를 담당합니다. 트레이로 숨긴 뒤에도 폴링이 이어집니다.
...
- **사용량 조회** — PC가 API를 다시 읽는 주기입니다. 기본 60초. 키보드에는 쓰지 않습니다.
- **화면 유지** — LCD에서 계정 화면이 다음으로 넘어가기 전에 머무는 시간입니다. 기본 10초.
- **키보드 기록** — 플래시에 다시 쓰는 최소 간격입니다. 기본 10분.
- **미리보기** — 업로드 없이 남은 %와 HUD를 확인합니다. 미리보기는 화면 유지 간격으로 돌아갑니다.
- **지금 키보드에 올리기** — 선택한 계정을 F108 Pro **사용자 GIF 슬롯**에 기록합니다.
