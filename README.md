...
- 업로드는 키보드 LCD용 SPI 플래시를 다시 씁니다. 기본은 **30분마다**만 기록합니다. 아래 **LCD 플래시 수명**을 보세요.
## 지원 계정
...
시간 기본값은 **60초 / 5초 / 30분**입니다. 세 값은 서로 다릅니다.
| 설정 | 기본값 | 하는 일 |
| --- | --- | --- |
| **사용량 조회** | 60초 | PC가 API에서 숫자를 다시 읽습니다. 키보드 플래시에는 쓰지 않습니다. |
| **계정당 표시 시간** | 5초 | 캐릭터 애니메이션을 포함한 계정 전체 화면의 총 시간입니다. 설정에서 변경할 수 있습니다. |
| **Flash 주기** | 30분 | 키보드 내부 저장공간에 다시 쓰는 최소 간격입니다. |

세 시간 항목은 숫자를 직접 입력할 수 있습니다. 저장·미리보기·업로드 때 범위를
벗어나거나 정수가 아니면 팝업으로 허용 범위를 안내하고 작업을 진행하지 않습니다.
사용량 조회는 15~3,600초, 계정당 표시 시간은 2~20초, Flash 주기는 1~720분입니다.
새 설정이나 시간 필드가 없는 설정은 5초와 30분을 사용합니다. 업그레이드할 때는
기존 `scene_hold_seconds` 값과 명시적인 Flash 주기를 보존하고, 이전 기본값 10분은
새 기본값 30분으로 자동 전환합니다.
...
기본 **Flash 주기는 30분**입니다. 하루 16시간 사용이면 시간당 2회, **하루 약 32회**입니다.
...
> **주의: 너무 빠르게 갱신하면 키보드 내부 저장공간의 수명이 감소합니다.** 기본 Flash 주기 30분을 유지하세요. **지금 키보드에 올리기**를 연속으로 눌러도 같은 슬롯을 반복해서 지웁니다.
## 지켜야 할 원칙
...
- **플래시를 너무 자주 쓰지 않습니다.** 기본 Flash 주기는 30분입니다. 너무 빠르게 하면 수명이 감소합니다.
## 더 읽기
