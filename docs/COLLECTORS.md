...
표시된다. HTTP 200에 현재 사용자 토큰 이벤트가 없어도 연결 검증과 자격
증명은 유효하지만 데이터 refresh 성공으로 기록하지 않는다. 이전 관측이
없으면 측정 0이 아닌 no-usage-yet(N/A) 상태를 보여 주고, 비어 있지 않은
last-known-good 캐시가 있으면 덮어쓰지 않고 stale로 유지한다. 실제 현재
사용자 토큰 이벤트를 받아 캐시를 성공적으로 저장한 경우에만 데이터
refresh 성공과 시간당 게이트의 성공 시각을 갱신한다. 캐시 저장에 실패하면
라이브 subtotal은 표시할 수 있어도 성공 시각은 갱신하지 않는다. 라이브
검증과 캐시/게이트 재사용은 구분되며, 인증/네트워크 실패는 후보 키를
저장하지 않는다. API 키는 `config.json`이 아니라 Windows 자격 증명 관리자에만
...
Admin 정규화는 현재 사용자에 귀속되고 토큰 사용량으로 해석되는 유효 이벤트를
최대 20,000개까지 보존한다. 제한 개수에 도달한 뒤 또 다른 유효 이벤트가
발견되면 관측 subtotal은 유지하되 `PARTIAL`과 제한 사유를 함께 표시한다.
중복 이벤트, 다른 사용자 이벤트, 토큰 이벤트가 아닌 행은 제한 초과로
세지 않는다. 이 completeness 정보는 Admin 캐시 v2에 저장되며, v1 또는
메타데이터가 없는 오래된 캐시는 보수적으로 partial로 표시해 새로고침을
유도한다. GUI와 `usage-engine --json`은 모두 설정의 `cursor_bindings`를
사용하므로 저장된 Cursor CSV/Admin 연결과 같은 계정 합계를 읽는다.
