...
- 이 사용자 장치의 F108 Pro에서 `/20` 패킹은 설정 5초가 약 1초, `/4` 패킹은 약 2.5초로 재생됐습니다. payload delay는 관측 재생 단위 2ms(`delay_byte = logical_ms / 2`)로 인코딩합니다. GUI/GIF 논리 시간은 설정한 밀리초 그대로입니다. 하드웨어 일반 사양이 아니므로 연결된 키보드에서 스톱워치 재측정이 필요합니다.
- 프레임당 최대 논리 delay는 500ms입니다. 기본 8계정 × 5초는 계정당 10프레임이 필요해 숨은 frame budget 기본값을 48에서 **80**으로 올렸고, 설정 스키마는 **config v8**입니다. 81~141이 필요한 유효 조합(8×6초=96, 9×5초=90, 3×20초=120)은 저장 전에 budget을 올리고, 141을 넘는 조합은 파일을 쓰기 전에 거절합니다. 프레임이 늘면 HID payload가 커집니다(80≈5.2MB, 96≈6.2MB, 141≈9.1MB).
- 구버전의 숨은 32와 v7 기본 48은 v8에서 80으로 마이그레이션합니다. 사용자가 명시한 다른 커스텀 budget은 보존한 뒤 1..141으로 clamp합니다.
- 완료 이력이 부족해 평균을 만들지 못하면 `BUILD` 대신 `NO AVG`를 씁니다. `NO AVG`는 AVG를 합성하지 않으며, GUI tooltip에 일간 완료 이력 n/7 또는 월간 완료 과거 월 n/1과 현재 기간이 시작일부터 완전한지를 표시합니다.
- GUI는 불가능한 계정×표시 시간 조합을 저장·미리보기·업로드 전에 한/영으로 안내하고, 필요량이 기본 80을 넘으면 숨은 budget을 올린 뒤 저장합니다. CLI도 같은 `SceneBudgetError`를 종료 코드 2로 닫습니다.
- `dist/QuotaDeck.exe`를 2026-09-17에 다시 빌드했습니다.
- 로컬 통합 검증: **524 passed, 1 skipped, 8 subtests passed** (고유 `--basetemp`). 건너뛴 1건은 Windows 디렉터리 심볼릭 링크 권한이 필요한 기존 테스트입니다.
